...
for i = 1:length(RANDOM_TISSUE_LIST)
    ...
    inFileName = strcat(tissue, '.network.degree.distribution');
    data = importdata(char(strcat(PATH_TO_DEGREE_DISTRIBUTION, inFileName)));
...
% Write to File
fileID = fopen(strcat(PATH_TO_DEGREE_DISTRIBUTION, 'average.network.degree.distribution'), 'w');
fprintf(fileID, '%s\t%f\n', 'Mean A', meanA);
fprintf(fileID, '%s\t%f\n', 'Mean K', meanK);
fprintf(fileID, '%s\t%f\n', 'Mean R^2', meanR);
fclose(fileID);